...
export class CommunityService {
  ...
  protected convertDateFromClient(community: ICommunity): ICommunity {
    return Object.assign({}, community, {
      lastModifyDate: community.lastModifyDate?.isValid() ? community.lastModifyDate.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((community: ICommunity) => {
        community.lastModifyDate = community.lastModifyDate ? dayjs(community.lastModifyDate) : undefined;
      });
    }
    return res;
  }
