...
export class CompanyPostComponent implements OnInit {
  ...
  trackId(index: number, item: ICompanyPost): number {
    return item.id!;
  }

  delete(companyPost: ICompanyPost): void {
    const modalRef = this.modalService.open(CompanyPostDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.companyPost = companyPost;
  ...
  protected onSuccess(data: ICompanyPost[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
