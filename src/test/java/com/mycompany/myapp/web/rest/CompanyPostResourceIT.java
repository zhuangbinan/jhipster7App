...
class CompanyPostResourceIT {
    ...
    @Test
    @Transactional
    void getAllCompanyPostsByCompanyUserIsEqualToSomething() throws Exception {
        // Initialize the database
        companyPostRepository.saveAndFlush(companyPost);
        CompanyUser companyUser = CompanyUserResourceIT.createEntity(em);
        em.persist(companyUser);
        em.flush();
        companyPost.addCompanyUser(companyUser);
        companyPostRepository.saveAndFlush(companyPost);
        Long companyUserId = companyUser.getId();

        // Get all the companyPostList where companyUser equals to companyUserId
        defaultCompanyPostShouldBeFound("companyUserId.equals=" + companyUserId);

        // Get all the companyPostList where companyUser equals to (companyUserId + 1)
        defaultCompanyPostShouldNotBeFound("companyUserId.equals=" + (companyUserId + 1));
    }
